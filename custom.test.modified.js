...
describe("Custom Tests", function () {
  ...
  it("[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances", async () => {
    const { tornadoPool, token, proxy, omniBridge, amb, gov, multisig } =
      await loadFixture(fixture);
    const aliceKeypair = new Keypair(); // contains private and public keys

    // Alice deposits in to tornado pool
    ...
    // Withdrawal address must have 0.08
  ...
  it("[assignment] iii. see assignment doc for details", async () => {
    ...
    const aliceEthAddress = "0xe7c10da45a629B0Dd6282f81Faa849D7cB161920";
    const aliceChangeUtxoAgain = new Utxo({
      amount: utils.parseEther("0.00"),
      keypair: aliceKeypair,
    });
    await transaction({
      tornadoPool,
      inputs: [aliceChangeUtxo],
      outputs: [aliceChangeUtxoAgain],
      recipient: aliceEthAddress,
      isL1Withdrawal: true, // set to false to withdraw from L2
    });
    ...
    expect(await token.balanceOf(aliceEthAddress)).to.be.equal(0);

    // Amount withdrawn to the L1
    const omniBridgeBalance = await token.balanceOf(omniBridge.address);
    expect(omniBridgeBalance).to.be.equal(utils.parseEther("0.07"));

    // amount remaining in Torando Pool
    const tornadoPoolBalance = await token.balanceOf(tornadoPool.address);
